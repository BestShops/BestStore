...
<body>
	...
	<div class="content inner">
		<section class="item-show__div item-show__head clearfix">
			<div class="pull-left">
				<ol class="breadcrumb" style="background: white;padding: 0">
					...
					<div class="item-pic__box" id="magnifier">
						<div class="small-box">
							<img style="width: 360px; height: 360px;"
								src="upload/${list.get(0).GPHOTOPIC }"
								alt="${list.get(0).GDESC }"> <span class="hover"></span>
						</div>

						<div>
							<br> <br> <a style="cursor: pointer;">商品来源：<strong>${list.get(0).sname }</strong></a>
							<c:if test="${collectionGoods.fstatus==1 }">
								<a onclick="removeCollectionGoods(${collectionGoods.fid})"
									style="float: right; color: #6c6c6c; cursor: pointer;">
							...
							<c:if test="${collectionGoods.fstatus==0 || collectionGoods==null }">
								<a onclick="collectionGoods(${list.get(0).GID })"
									style="float: right; color: #6c6c6c; cursor: pointer;"> <img
									src="${basePath }/images/xing1.png"
									style="width: 15px; height: 15px; margin-right: 5px; margin-top: -5px">收藏宝贝
								</a>
							</c:if>
						</div>
						<div class="big-box">
							<img src="upload/${list.get(0).GPHOTOPIC }"
								alt="${list.get(0).GDESC }">
						</div>
					</div>
				...
				$(function () {
					...
					<div class="item-info__box">
						<div class="item-title">
							<div class="name ep2">
								<input name="gid" value="${list.get(0).GID}" type="hidden"
									readonly="readonly"
						...
						<div class="item-price bgf5">
							<div class="price-box clearfix">
								<div class="price-panel pull-left">
									售价：<span class="price">￥<span
										style="width: 80px; height: 20px; border: 0px; outline: none;">${list.get(0).GNOWPRICE}</span>
										<input type="hidden" name="gprice"
										value="${list.get(0).GNOWPRICE}" /> <s class="fz16 c9">￥${list.get(0).GLASTPRICE}</s></span>
								</div>
								<script>
							// 会员价格折叠展开
							$(function () {
						...
						<ul class="item-ind-panel clearfix">
							<li class="item-ind-item"><span class="ind-label c9">累计销量</span>
								...
								</c:if> <c:if test="${list.get(0).num==null }">
									<span class="ind-count cr">0</span>
								</c:if></li>
							<li class="item-ind-item"><a href=""><span
									class="ind-label c9">累计评论</span> <c:if
										test="${list.get(0).count==null }">
										<span class="ind-count cr">0</span>
									</c:if> <c:if test="${list.get(0).count!=null }">
										<span class="ind-count cr">${list.get(0).count }</span>
									</c:if> </a></li>
							<li class="item-ind-item"><span class="ind-label c9">商品评分</span>
								<span class="ind-count cr">${list.get(0).GRADE }</span></li>
						</ul>
						<div class="item-key">
							<div class="item-sku">
									...
									<dd>
										<ul data-property="颜色" class="clearfix">
											<li><c:if test="${color==1}">
													<a class="on"
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=1&size=${size}"
														role="button" data-value="白色" aria-disabled="true"> <span>白色</span></a>
												</c:if> <c:if test="${color!=1}">
													<a
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=1&size=${size}"
														role="button" data-value="白色" aria-disabled="true"> <span>白色</span></a>
												</c:if></li>
											<li><c:if test="${color==2 }">
													<a class="on"
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=2&size=${size}"
														role="button" data-value="黑色" aria-disabled="true"> <span>黑色</span></a>
												</c:if> <c:if test="${color!=2 }">
													<a
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=2&size=${size}"
														role="button" data-value="黑色" aria-disabled="true"> <span>黑色</span></a>
												</c:if></li>
											<li><c:if test="${color==3 }">
													<a class="on"
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=3&size=${size}"
														role="button" data-value="粉红色" aria-disabled="true"> <span>粉红色</span>
													</a>
												</c:if> <c:if test="${color!=3 }">
													<a
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=3&size=${size}"
														role="button" data-value="粉红色" aria-disabled="true"> <span>粉红色</span>
													</a>
												</c:if></li>
											<li><c:if test="${color==4 }">
													<a class="on"
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=4&size=${size}"
														role="button" data-value="黄色" aria-disabled="true"> <span>黄色</span>
													</a>
												</c:if> <c:if test="${color!=4 }">
													<a
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=4&size=${size}"
														role="button" data-value="黄色" aria-disabled="true"> <span>黄色</span>
													</a>
								...
								<dl class="item-prop clearfix">
									...
									<dd>
										<ul data-property="尺码" class="clearfix">
											<li><c:if test="${size==1}">
													<a class="on"
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=${color}&size=1"
														role="button"> <span>S</span>
													</a>
												</c:if> <c:if test="${size!=1}">
													<a
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=${color}&size=1"
														role="button" data-value="S" aria-disabled="true"> <span>S</span>
													</a>
												</c:if></li>
											<li><c:if test="${size==2 }">
													<a class="on"
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=${color}&size=2"
														role="button" data-value="M" aria-disabled="true"> <span>M</span>
													</a>
												</c:if> <c:if test="${size!=2 }">
													<a
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=${color}&size=2"
														role="button" data-value="M" aria-disabled="true"> <span>M</span>
													</a>
											...
											<li><c:if test="${size==3 }">
													<a class="on"
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=${color}&size=3"
														role="button" data-value="L" aria-disabled="true"> <span>L</span>
													</a>
												</c:if> <c:if test="${size!=3 }">
													<a
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=${color}&size=3"
														role="button" data-value="L" aria-disabled="true"> <span>L</span>
													</a>
											...
											<li><c:if test="${size==4 }">
													<a class="on"
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=${color}&size=4"
														role="button" data-value="XL" aria-disabled="true"> <span>XL</span>
													</a>
												</c:if> <c:if test="${size!=4}">
													<a
														href="goodsShowPage.todo?gid=${list.get(0).GID }&color=${color}&size=4"
														role="button" data-value="XL" aria-disabled="true"> <span>XL</span>
													</a>
							...
							<div class="item-amount clearfix bgf5">
								...
								<div class="amount-box">
									<div class="amount-widget">
										<input class="amount-input" id="buy_goodsNum" name="num"
											min="1" max="${list.get(0).GNUMBER }" value="1" type="number">
							...
							<div class="item-action clearfix bgf5">
								<input type="submit" value="立即购买" rel="nofollow"
									data-addfastbuy="true" title="点击此按钮，到下一步确认购买信息。" role="button"
									class="item-action__buy"> <a id="addCart"
					...
					<div class="swiper-wrapper">
						...
						<c:forEach items="${linkList}" var="ll">
							<div class="swiper-slide">
								<a class="picked-item" href=""> <img
									src="upload/${ll.GPHOTOPIC }" alt="" class="cover">
				...
				<div class="tab-content">
					...
					<div role="tabpanel" class="tab-pane fade in active" id="detail"
						...
						<div class="rich-text">
							<p style="text-align: center;">
								<i id="desc-module-1" style="font-size: 0"></i> <img
									src="upload/${list.get(0).GPHOTOPIC }" alt=""><br> <br>
								<i id="desc-module-2" style="font-size: 0"></i> <a>${list.get(0).GDESC }</a><br>
								<br> <i id="desc-module-3" style="font-size: 0"></i> <img
									src="upload/${list.get(0).GPHOTOPIC }" alt=""><br> <br>
								<i id="desc-module-4" style="font-size: 0"></i> <a>${list.get(0).GDESC }</a><br>
								<br> <i id="desc-module-5" style="font-size: 0"></i> <img
									src="upload/${list.get(0).GPHOTOPIC }" alt=""><br> <br>
							</p>
						</div>
					</div>
					...
					<div role="tabpanel" class="tab-pane fade" id="service"
						...
						<div class="service-content rich-text">
							...
							<h3>一、全国联保</h3>
							<p style="text-indent: 2em">AOC
								显示器实施全国范围联保，国家标准三包服务。无论您是在中国大陆 ( 不含香港、澳门、台湾地区)
								何处购买并在大陆地区使用的显示器，出现三包范围内的故障时，可凭显示器的保修证正本和购机发票到 AOC
			...
			<div class="pull-right">
				<div class="tab-content" id="descCate">
					<div role="tabpanel" class="tab-pane fade in active"
						id="detail-tab" aria-labelledby="detail-tab">
